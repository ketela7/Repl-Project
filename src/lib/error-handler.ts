...
class ErrorHandler {
  ...
  private isRetryableError(error: unknown, currentCount: number): boolean {
    ...
    const errorCode = this.extractErrorCode(error)
    const status =
      (error as any)?.response?.status || (error as any)?.status || (error as any)?.error?.code || 0
...
export function handleError(error: unknown): ErrorResponse {
  ...
  if (error instanceof AppError) {
    ...
    // Severity-based error handling - logging removed to comply with ESLint strict mode
    // Error details are captured in the response for proper error handling
