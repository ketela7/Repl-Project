...
function parseDriveUrl(url: string): { folderId: string | null; isValid: boolean } {
  try {
    ...
    const patterns = [/drive\.google\.com\/drive\/(?:u\/\d+\/)?folders\/([a-zA-Z0-9_-]+)/, /drive\.google\.com\/open\?id=([a-zA-Z0-9_-]+)/, /drive\.google\.com\/drive\/folders\/([a-zA-Z0-9_-]+)/]
...
export function DriveDestinationSelector({ onSelect, selectedFolderId = 'root', className }: DriveDestinationSelectorProps) {
  const [activeTab, setActiveTab] = useState<'browse' | 'url'>('browse')
  const [urlInput, setUrlInput] = useState('')
  const [parsedResult, setParsedResult] = useState<{ folderId: string | null; isValid: boolean }>({ folderId: null, isValid: false })
  const [isValidating, setIsValidating] = useState(false)
  const [validationResult, setValidationResult] = useState<{ isValid: boolean; folderName?: string; error?: string } | null>(null)
  ...
  const filteredFolders = searchQuery ? folders.filter((folder) => folder.name.toLowerCase().includes(searchQuery.toLowerCase())) : folders
      ...
      <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as 'browse' | 'url')}>
        ...
        <TabsContent value="browse" className="space-y-4">
          ...
          <div className="relative">
            ...
            <Input placeholder="Search folders..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="h-8 pl-7 text-xs" />
          </div>
          ...
          <ScrollArea className="h-64 rounded-md border">
            <div className="space-y-1 p-2">
              ...
              {filteredFolders.length > 20 && <div className="text-muted-foreground py-1 text-center text-xs">... and {filteredFolders.length - 20} more folders</div>}
            </div>
          </ScrollArea>
        </TabsContent>
        ...
        <TabsContent value="url" className="space-y-4">
          <div className="space-y-2">
            ...
            <Input
              ...
              className={cn(parsedResult.isValid && 'border-green-500', urlInput && !parsedResult.isValid && 'border-red-500')}
            />
            <div className="text-muted-foreground text-xs">Supports: Full Google Drive URLs, folder sharing links, or direct folder IDs</div>
          </div>
              ...
              {parsedResult.isValid ? (
                <div className="flex items-start gap-2 rounded-lg border border-green-200 bg-green-50 p-3 dark:border-green-800 dark:bg-green-950/20">
                  ...
                  <div className="flex-1 space-y-2">
                    ...
                    <div className="rounded border bg-green-100 p-2 font-mono text-xs dark:bg-green-900/30">Folder ID: {parsedResult.folderId}</div>
                    {!validationResult && (
                      <Button size="sm" onClick={() => validateFolderId(parsedResult.folderId!)} disabled={isValidating} className="w-full">
                        {isValidating ? (
                          <>
                ...
                <div className="flex items-start gap-2 rounded-lg border border-red-200 bg-red-50 p-3 dark:border-red-800 dark:bg-red-950/20">
                  ...
                  <div className="text-sm text-red-800 dark:text-red-200">Invalid URL format. Please check the URL and try again.</div>
                </div>
              )}
              ...
              {validationResult && (
                <div
                  className={cn(
                    ...
                    validationResult.isValid ? 'border-blue-200 bg-blue-50 dark:border-blue-800 dark:bg-blue-950/20' : 'border-red-200 bg-red-50 dark:border-red-800 dark:bg-red-950/20'
                  )}
                >
                  ...
                  <div className="flex-1 space-y-2">
                    <div className={cn('text-sm', validationResult.isValid ? 'text-blue-800 dark:text-blue-200' : 'text-red-800 dark:text-red-200')}>
                      {validationResult.isValid ? `Folder found: "${validationResult.folderName}"` : validationResult.error}
                    </div>
                    {validationResult.isValid && (
