...
function ItemsUntrashDialog({ isOpen, onClose, onConfirm, selectedItems }: ItemsUntrashDialogProps) {
  ...
  const handleUntrash = async () => {
    ...
    try {
      ...
      if (!isCancelledRef.current) {
        if (successCount > 0) {
          toast.success(`Restored ${successCount} item${successCount > 1 ? 's' : ''} from trash`)
          onConfirm()
        }
        if (failedCount > 0) {
          toast.error(`Failed to restore ${failedCount} item${failedCount > 1 ? 's' : ''}`)
        }
      }
    } catch (err) {
      if (abortControllerRef.current?.signal.aborted) {
      ...
      console.error(err)
      toast.error('Untrash operation failed')
    } finally {
