...
export function FilePreviewDialog({ open, onOpenChange, file }: FilePreviewDialogProps) {
  ...
  const renderPreviewContent = () => {
    ...
    const containerClasses = isFullscreen ? 'bg-white h-screen w-screen' : 'bg-white rounded-lg border overflow-hidden'
        ...
        <div className="absolute top-0 right-0 left-0 z-10 bg-black/80 p-4 backdrop-blur-sm">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <h2 className="max-w-md truncate font-medium text-white">{file.name}</h2>
              <div className="hidden text-sm text-white/70 sm:block">{file.mimeType}</div>
            </div>
            <div className="flex items-center gap-2">
              ...
              <Button variant="ghost" size="sm" onClick={handleOpenInDrive} className="text-white hover:bg-white/10">
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="h-auto max-h-[95vh] w-[95vw] max-w-6xl overflow-hidden p-3 sm:p-4 md:p-6">
            ...
            <div className="flex flex-shrink-0 items-center gap-1 sm:gap-2">
              ...
              {file.webContentLink && (
                <>
                  ...
                  <Button variant="outline" size="sm" onClick={handleDownload} className="h-8 w-8 p-0 sm:hidden">
                    <Download className="h-4 w-4" />
                  </Button>
                </>
              )}
              {/* Open in Drive */}
              <Button variant="outline" size="sm" onClick={handleOpenInDrive} className="hidden sm:flex">
                <ExternalLink className="mr-2 h-4 w-4" />
                Open in Drive
              </Button>
              <Button variant="outline" size="sm" onClick={handleOpenInDrive} className="h-8 w-8 p-0 sm:hidden">
        ...
        <div className="mt-1 flex-1 overflow-hidden sm:mt-2">{renderPreviewContent()}</div>
        ...
        <div className="text-muted-foreground flex flex-col gap-1 border-t pt-3 text-xs sm:flex-row sm:items-center sm:justify-between sm:gap-0 sm:text-sm">
          ...
          {file.size && <span className="text-right">Size: {formatFileSize(file.size)}</span>}
        </div>
      </DialogContent>
    </Dialog>
  )
